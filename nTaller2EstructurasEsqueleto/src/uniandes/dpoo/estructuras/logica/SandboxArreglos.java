...
public class SandboxArreglos
{
    ...
    public String[] getCopiaCadenas( )
    {
    	...
    	String[] copia = new String[tamano];    
    ...
    {
    	int tamano = arregloEnteros.length;
    	if (tamano == arregloEnteros.length) {
    ...
    {
    	int tamano = arregloCadenas.length;
    	if (tamano == arregloCadenas.length) {
    ...
    public int[] buscarEntero(int valor) {
        ...
        int[] posiciones = new int[coincidencias];
        int index = 0;
    ...
    public int contarEnterosRepetidos() {
        if (arregloEnteros.length == 0) return 0;

        HashMap<Integer, Integer> freq = new HashMap<>();
        for (int i = 0; i < arregloEnteros.length; i++) {
            int v = arregloEnteros[i];
            Integer actual = freq.get(v);
            if (actual == null) {
                freq.put(v, 1);
            } else {
                freq.put(v, actual + 1);
        ...
        int repetidos = 0;
        Object[] valores = freq.values().toArray();
        for (int i = 0; i < valores.length; i++) {
            int conteo = (Integer) valores[i];
            if (conteo > 1) {
                repetidos++;
            }
        }
    ...
    public boolean mismosEnteros(int[] otroArreglo) {
        ...
        for (int i = 0; i < arregloEnteros.length; i++) {
            int val = arregloEnteros[i];
            Integer actual = freq1.get(val);
            freq1.put(val, (actual == null) ? 1 : actual + 1);
        }

        for (int i = 0; i < otroArreglo.length; i++) {
            int val = otroArreglo[i];
            Integer actual = freq2.get(val);
            freq2.put(val, (actual == null) ? 1 : actual + 1);
        }

        Object[] llaves = freq1.keySet().toArray();
        if (llaves.length != freq2.size()) return false;

        for (int i = 0; i < llaves.length; i++) {
            int clave = (Integer) llaves[i];
            Integer f1 = freq1.get(clave);
            Integer f2 = freq2.get(clave);
    ...
    public void generarEnteros(int cantidad, int minimo, int maximo) {
        ...
        for (int i = 0; i < cantidad; i++) {
            int val = minimo + (int) (Math.random() * rango);
            arregloEnteros[i] = val;
        }
    }
